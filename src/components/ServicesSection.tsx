import { Globe, Palette, Layers, ArrowRight } from "lucide-react";
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service, index) => (
            <Link
              key={index}
              href={service.href}
              className="group relative bg-zinc-900 rounded-2xl border border-zinc-800 p-8 transition-all duration-500 hover:border-blue-600/50 hover:shadow-2xl hover:shadow-blue-600/10 hover:-translate-y-1 block"
            >
              {/* Icon */}
              ...
              {/* Learn More */}
              <span className="inline-flex items-center gap-2 text-blue-500 group-hover:text-blue-400 font-medium text-sm transition-colors">
                Learn More
